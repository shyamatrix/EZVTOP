...
router.get("/", async (_req, res) => {
    try {
        ...
        const statsDocs = [];
        for (const entry of Logger_1.inMemoryStats.values()) {
        ...
        const hourlyData = statsDocs.map((doc) => {
        ...
        const hourCounts = hourlyData.map((d) => Number(d.total));
        const routeKeysSet = new Set();
        const sourceKeysSet = new Set();
        ...
        function decodeRouteField(field) {
            if (field === "unknown")
                return "unknown";
            return field
                .replace(/_/g, "/")
        ...
        function decodeSourceField(field) {
