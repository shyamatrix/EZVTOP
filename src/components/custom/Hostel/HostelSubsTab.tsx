...
interface HostelSubTabsProps {
  HostelActiveSubTab: string;
...
export default function HostelSubTabs({
  ...
  return (
    <div className="flex items-center p-1 bg-white/20 dark:bg-zinc-950/40 backdrop-blur-md border border-white/10 dark:border-zinc-800/40 rounded-full w-full max-w-[320px] mx-auto mb-6 shadow-md">
      {tabs.map((tab) => {
        const isActive = HostelActiveSubTab === tab.id;
        return (
          <button
            key={tab.id}
            onClick={() => setHostelActiveSubTab(tab.id)}
          ...
          >
            {isActive && (
              <motion.div
                layoutId="activeHostelSubTabBubble"
